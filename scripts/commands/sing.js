...
const handleReply = async ({ api, event, handleReply }) => {
  try {
    const songIndex = parseInt(event.body);
    if (!Number.isInteger(songIndex) || songIndex <= 0 || songIndex > handleReply.link.length) {
      return api.sendMessage("Invalid song index! Please choose a valid song number.", event.threadID, event.messageID);
    }

    const path = `${__dirname}/cache/audio-${event.senderID}.mp3`;
    const { data, info } = await downloadMusicFromYoutube("https://www.youtube.com/watch?v=" + handleReply.link[songIndex - 1], path);
